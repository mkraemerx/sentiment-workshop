...
trn_ds, val_ds, tst_ds = full_ds.split(
    split_ratio=[0.8, 0.1, 0.1], stratified=True, random_state=random.seed(SEED))
...
def train(model, iterator, optimizer, criterion, metric):
    ...
    for batch in iterator:
        optimizer.zero_grad()
        y_hat = model(batch.lemma).squeeze(1)
...
# RNN variant SETUP
model = SimpleLSTM(len(f_lemma.vocab), EMB_SIZE, HID_SIZE,
                   NUM_RNN_LAYERS, bidirectional=BIDIRECTIONAL, dropout=DROPOUT)
optimizer = optim.Adam(model.parameters())
criterion = nn.BCEWithLogitsLoss()

model.embedding.weight.data.copy_(f_lemma.vocab.vectors)
